"""
SKAB Deep Learning Robustness Deneyleri
- Gaussian Noise senaryosu
- Unseen Data senaryosu

Mevcut run_skab_seed_experiments.py dosyasını değiştirmez.
Yeni CSV çıktıları:
  results/outputs/skab_dl_robustness_results.csv
  results/outputs/skab_dl_robustness_summary.csv
"""
...
def run_skab_robustness(model_type, fold_id, seed):
    cfg = get_dl_config()

    print(f"\n=== SKAB {model_type} fold={fold_id} seed={seed} robustness ===")
    ...
    results.append({
        "dataset": "SKAB", "model": model_type, "seed": seed, "fold": fold_id,
    ...
    train_mean = X_tr.mean(axis=(0, 1))
    train_std  = X_tr.std(axis=(0, 1)) + 1e-8
...
def main():
    ...
    results_df.to_csv(output_dir / "skab_dl_robustness_results.csv", index=False)

    summary_df = summarize_results(results_df)
    summary_df.to_csv(output_dir / "skab_dl_robustness_summary.csv", index=False)

    print("\n=== SKAB Deep Learning Robustness Özet ===")

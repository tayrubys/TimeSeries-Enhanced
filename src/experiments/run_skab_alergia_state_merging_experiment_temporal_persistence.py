    ...
    return {
        ...
        "min_anomaly_runs": [
            ...
            for value in automata.get(
                "alergia_skab_min_anomaly_run_values",
                [1, 2, 3, 4, 5, 6, 7, 8],
            )
        ],
        "min_validation_recall": float(
            automata.get(
                "alergia_skab_min_validation_recall",
                0.70,
            )
        ),
    }
...
def apply_temporal_persistence(predictions, min_anomaly_run):
    """
    En az min_anomaly_run uzunluğundaki ardışık anomaly bloklarını
    korur. Daha kısa anomaly bloklarını normal yapar.
    """
    ...
    if min_anomaly_run < 1:
        raise ValueError(
            "min_anomaly_run en az 1 olmalıdır."
        )
...
def apply_grouped_temporal_persistence(
    raw_predictions,
    grouped_data,
    min_anomaly_run,
):
    """
    Temporal Persistence Filter'ı her source_file için ayrı uygular.
    Bir dosyanın sonundaki tahminler diğer dosyanın başıyla birleşmez.
    """
    ...
    for item in grouped_data:
        file_length = len(item["transition_labels"])
        file_predictions = raw_predictions[
            offset : offset + file_length
        ]

        if len(file_predictions) != file_length:
            raise ValueError(
                f"{item['source_file']} için tahmin sayısı eksik."
            )
    ...
    if offset != len(raw_predictions):
        raise ValueError(
            "Grouped persistence sırasında toplam tahmin sayısı "
            "grouped_data ile uyuşmuyor."
        )
...
def select_threshold_with_grouped_persistence(
    labels,
    scores,
    grouped_data,
    min_anomaly_runs,
    ...
    """
    Threshold ve min_anomaly_run değerini yalnızca inner validation
    üzerinden birlikte seçer.

    Önce validation recall alt sınırını sağlayan ayarlar tutulur.
    Bunlar arasında F1 değeri en yüksek olan seçilir. Hiçbir ayar
    recall sınırını sağlayamazsa en yüksek recall değerli ayar
    güvenli fallback olarak kullanılır.
    """
    ...
    expected_count = sum(
        len(item["transition_labels"])
        for item in grouped_data
    )

    if expected_count != len(scores):
        raise ValueError(
            "Grouped validation uzunluğu skor sayısıyla uyuşmuyor."
        )
    ...
    for threshold in candidates:
        ...
        for min_anomaly_run in run_values:
            filtered_predictions = (
                apply_grouped_temporal_persistence(
                    raw_predictions,
                    grouped_data,
                    min_anomaly_run,
                )
...
def make_result_row(
    ...
    return {
        ...
        "min_anomaly_run": best["config"]["min_anomaly_run"],
        "min_validation_recall": best["config"][
            "min_validation_recall"
        ],
...
def run_fold_seed(fold_id, seed, config, transformer):
                ...
                validation_rows.append(
                    {
                        ...
                        **kwargs,
                        ...
                        "selection_mode": "state_merging_baseline",
                        "threshold": threshold,
    ...
    print(
        "\n--- SOURCE_FILE BAZLI TEMPORAL PERSISTENCE TARAMASI ---"
    )
    ...
    for distance_penalty in config["distance_penalties"]:
        ...
        selection = select_threshold_with_grouped_persistence(
            val_labels,
            val_scores,
            split_info["grouped_val"],
        ...
        validation_rows.append(
            {
                ...
                "window_size": config["window_size"],
                "alphabet_size": config["alphabet_size"],
                "merge_alpha": best["config"][
                    "merge_alpha"
                ],
                "min_state_count": best["config"][
                    "min_state_count"
                ],
                "smoothing_alpha": best["config"][
                    "smoothing_alpha"
                ],
    ...
    test_predictions = apply_grouped_temporal_persistence(
        test_raw_predictions,
        grouped_test,
    ...
    for raw_prediction, prediction, log in zip(
        ...
        log["threshold"] = float(best["threshold"])
        log["min_anomaly_run"] = int(
            best["config"]["min_anomaly_run"]
        )
        log["raw_decision"] = (
    ...
    noisy_raw_predictions = (
        noisy_scores >= best["threshold"]
    ).astype(int)
    noisy_predictions = apply_grouped_temporal_persistence(
        noisy_raw_predictions,
        grouped_noisy_test,
        best["config"]["min_anomaly_run"],
    )
    ...
    print(
        f"Test raw/filtered anomaly: "
        f"{int(test_raw_predictions.sum())}/"
        f"{int(test_predictions.sum())} | "
        f"actual anomaly={int(test_labels.sum())}"
    )
